...
void impCliente(Cliente C,char primeiroNome[50],char segundoNome[50],char estado[50]){
	...
	printf("\nNome=%s %s   Idade=%d  Cpf=%d  Estado=%s  ID=%d",C.primeiroNome,C.segundoNome,C.idade,C.cpf,C.estado,C.id);
}

void impProduto(Produto P,char nomeProduto[50]){
	setlocale(LC_ALL,"portuguese");
	strcpy(P.nomeProduto,nomeProduto);
	printf("\nProduto=%s  Preço=%d  Quantidade=%d  %d",P.nomeProduto,P.preco,P.quantidade,P.id);
}
...
void cadastraProduto(Produto P,char nomeProduto[50]){
	...
	if(produto == NULL){
		...
		fprintf(produto,"\nProduto=%s  Preco=%d  Quantidade=%d  %d",P.nomeProduto,P.preco,P.quantidade,P.id);
	};
	fclose(produto);
}
